# encoding: utf-8

module ChineseLunar
  # 公历农历互转
  class Lunar

    LunarInfo = [
      0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554,
      0x056a0, 0x09ad0, 0x055d2, 0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250,
      0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977, 0x04970,
      0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570,
      0x052f2, 0x04970, 0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0,
      0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950, 0x0d4a0, 0x1d8a6,
      0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950,
      0x0b557, 0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5d0, 0x14573,
      0x052d0, 0x0a9a8, 0x0e950, 0x06aa0, 0x0aea6, 0x0ab50, 0x04b60,
      0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
      0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558,
      0x0b540, 0x0b5a0, 0x195a6, 0x095b0, 0x049b0, 0x0a974, 0x0a4b0,
      0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570, 0x04af5,
      0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x055c0, 0x0ab60,
      0x096d5, 0x092e0, 0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552,
      0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5, 0x0a950, 0x0b4a0,
      0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0,
      0x0a930, 0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6,
      0x0a4e0, 0x0d260, 0x0ea65, 0x0d530, 0x05aa0, 0x076a3, 0x096d0,
      0x04bd7, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
      0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50,
      0x1b255, 0x06d20, 0x0ada0
    ].freeze

    Gan = %w(甲 乙 丙 丁 戊 己 庚 辛 壬 癸).freeze
    Zhi = %w(子 丑 寅 卯 辰 巳 午 未 申 酉 戌 亥).freeze

    attr_reader :Date

    def initialize(date = Date.current)
      @date = date
    end

    # Get the Lundar date in 'xxxx-xx-xx' fromat
    def lunar_date
      lunar = convert(@date.year, @date.month, @date.day)
      lunar[0].to_s + '-' + month(lunar[1]) + '-' + day(lunar[2])
    end

    private

    def month(month)
      month.to_s.rjust(2, '0')
    end

    def day(day)
      /^\d+/.match(day.to_s).to_s.rjust(2, '0')
    end

    def cyclical_year(year)
      num = year - 1900 + 36
      cyclicalm(num)
    end

    def cyclicalm(num)
      Gan[num % 10] + Zhi[num % 12]
    end

    # 传出y年m月d日对应的农历.
    # year0 .month1 .day2 .yearCyl3 .monCyl4 .dayCyl5 .isLeap6
    def convert(year, month, day)
      nong_date = [] # 初始化农历成一个数组，存(year, month, day)
      count = 0 # 初始化一个计数器
      temp = 0 # 初始化一个临时变量
      leap = 0 # 初始化一个堆
      base_date = Date.new(0 + 1900, 1, 31)

      obj_date = Date.new(year, month, day);
      offset = obj_date - base_date
      nong_date[5] = offset + 40;
      nong_date[4] = 14;

      count = 1900
      while count < 2050 && offset > 0
        temp = days_in_lunar_date(count)
        offset -= temp
        nong_date[4] += 12

        count += 1
      end

      if (offset < 0)
        offset += temp
        count -= 1
        nong_date[4] -= 12
      end

      nong_date[0] = count
      nong_date[3] = count - 1864
      leap = leap_month(count) #闰哪个月
      nong_date[6] = 0

      count = 1  
      while(count < 13 && offset > 0)
        #闰月
        if (leap > 0 && count == (leap + 1) && nong_date[6] == 0)
          count -= 1
          nong_date[6] = 1
          temp = leap_days( nong_date[0])
        else
          temp = monthDays( nong_date[0], count)
        end

        #解除闰月
        if (nong_date[6] == 1 && count == (leap + 1))
          nong_date[6] = 0
        end
        offset -= temp
        if (nong_date[6] == 0)
          nong_date[4] += 1
        end

        count += 1
      end

      if (offset == 0 && leap > 0 && count == leap + 1)
        if (nong_date[6] == 1)
          nong_date[6] = 0
        else
          nong_date[6] = 1
          count -= 1
          nong_date[4] -= 1
        end
      end

      if (offset < 0)
        offset += temp
        count -= 1
        nong_date[4] -= 1
      end

      nong_date[1] = count
      nong_date[2] = offset + 1

      nong_date
    end
    # ********** END **********

    # Return the days in lunar of y year.
    def days_in_lunar_date(year)
      sum = 348
      count = 0x8000
      while count > 0x8
        if ((LunarInfo[year - 1900] & count) != 0)
          sum += 1
        end
        count >>= 1
      end

      sum + leap_days(year)
    end

    # Return the leap days in y year.
    def leap_days(y)
      if (leap_month(y) != 0)
        if ((LunarInfo[y - 1900] & 0x10000) != 0)
          return 30
        else
          return 29
        end
      else
        return 0
      end
    end

    # 传回农历 y年闰哪个月 1-12 , 没闰传回 0  
    def leap_month(y)
      LunarInfo[y - 1900] & 0xf
    end

    # Return the days of m month in y year.
    def  monthDays(y, m)
      if ((LunarInfo[y - 1900] & (0x10000 >> m)) == 0)
        return 29
      else
        return 30
      end
    end

  end
end
