...
class BirthdayMessageJob < ApplicationJob
  ...
  def perform(user_id)
    ...
    receivers.each do |receiver|
      feedback = ChinaSMS.to receiver.mobile, %Q(【高老庄】亲爱的#{reciever.name}，今天是你的亲人#{user.name}，#{user.age}岁的生日，请对#{user.zh_sex}说声“生日快乐”，方便的话，也不要吝啬的你的双手，给#{user.zh_sex}一个真诚的拥抱。)
      if feedback["code"] == 0
